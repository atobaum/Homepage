...
class Page extends IPage {
    ...
    static async load(fulltitle) {
        ...
        let res = await SingletonMysql_1.default.query("SELECT * FROM fullpage WHERE ns_title = ? and page_title = ?", titles);
        let rows = res[0];
        if (rows.length === 0) {
            res = await SingletonMysql_1.default.query("SELECT * FROM namespace WHERE ns_title = ?", [titles[0]]);
            rows = res[0];
            if (rows.length === 0)
            ...
            let data = rows[0];
            res = await SingletonMysql_1.default.query('SELECT * FROM wiki_tags WHERE wiki_id=?', [data.page_id]);
            rows = res[0];
...
class OldPage extends Page {
    ...
    async getSrc(user) {
        ...
        if (this.srcStr)
            ...
            let rows = await SingletonMysql_1.default.query('SELECT * FROM revision WHERE page_id = ? AND rev_id = ?', [this.pageId, this.revId]);
