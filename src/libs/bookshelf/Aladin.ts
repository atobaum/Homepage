...
export default class Aladin {
    ...
    private static host = 'http://www.aladin.co.kr/ttb/api/';
    ...
    parseAuthors(strAuthors) {
        ...
        authors.forEach(function(strAuthor){
            let type: any = typeRegex.exec(strAuthor);
    ...
    async bookInfo(isbn) {
        ...
        try {
            ...
            let item: any = await request(queryOption);
            if (item.errorCode) {
     ...
     */
    async search(type, keyword): Promise<Book[]> {
        ...
        let data: any[] = (await request(queryOption) as any).item;
